...
class MotionLoader:
  ...
  def _load_motion(self):
    """Loads the motion from the qpos file."""
    motion =  torch.from_numpy(np.load(self.motion_file)['qpos'])
...
def main(
  ...
  """Replay motion from QPOS file and output to npz file.

  Args:
    input_file: Path to the input QPOS file.
    output_name: Path to the output npz file.
    input_fps: Frame rate of the QPOS file.
    output_fps: Desired output frame rate.
    device: Device to use.
    render: Whether to render the simulation and save a video.
    line_range: Range of lines to process from the QPOS file.
  """
